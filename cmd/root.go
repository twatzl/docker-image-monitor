...
import (
	...
	"github.com/twatzl/go-cmd-skeleton/app"
)
...
var rootCmd = &cobra.Command{
	Use:   "image-monitor",
	Short: "image-monitor will check for which docker containers and images updates are available",
	Long:  `TODO`,
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.image-monitor.yaml)")
...
func initConfig() {
	...
	if cfgFile != "" {
		...
		// Search config in home directory with name ".image-monitor" (without extension).
		viper.AddConfigPath(home)
		viper.SetConfigName(".image-monitor")
	}
